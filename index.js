...
app.post('/ads', verifyAuth, async (req, res) => {
  const { title, description, image_url, category, location, extra } = req.body
  const userId = req.user.uid

  try {
    const insertAd = await pool.query(
      'INSERT INTO ads (title, description, image_url, category, location, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
      [title, description, image_url, category, location, userId]
    )
